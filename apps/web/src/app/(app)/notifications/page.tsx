    ...
    <div className="max-w-xl mx-auto">
      ...
      <header className="sticky top-0 z-10 glass border-b border-slate-100 px-4 h-14 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <h1 className="text-lg font-bold text-slate-900">Bildirimler</h1>
          {unreadCount > 0 && (
      ...
      <div className="divide-y divide-slate-50">
        {isLoading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-sm text-slate-400 mt-1">Aktivite ve etkileşimler burada görünür</p>
          </div>
        ) : (
          notifications.map(notif => (
            <button
              ...
              className={`w-full flex items-start gap-3 px-4 py-3.5 hover:bg-slate-50 transition text-left
                ${!notif.is_read ? 'bg-amber-50/50' : 'bg-white'}`}
            >
              {/* Avatar / icon */}
              <div className="relative shrink-0">
                <div className="w-11 h-11 rounded-full bg-slate-100 flex items-center justify-center text-xl overflow-hidden">
                ...
                {(notif as any).actor?.avatar_url && (
                  <span className="absolute -bottom-0.5 -right-0.5 w-5 h-5 bg-white rounded-full flex items-center justify-center text-xs border border-slate-100 shadow-sm">
                    {TYPE_ICONS[notif.type] ?? '📢'}
                  </span>
                )}
              </div>
              ...
              {/* Unread dot */}
              {!notif.is_read && (
                <span className="shrink-0 w-2.5 h-2.5 bg-amber-500 rounded-full mt-1.5" />
              )}
            </button>
          ))
        )}
